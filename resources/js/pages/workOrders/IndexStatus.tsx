...
const WorkOrderIndexPage: React.FC<Props> = ({ status, rules, printStatusOptions, operatorOptions, printerOptions, productList, workOrderDetails }) => {
  ...
  const onFilterChange = (e: any) => {
    ...
    router.get(route('work-orders-status.index', status), {
      filters: e.filters,
    }, { 
      // preserveState: true,
    });
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    { title: __('workOrder.workOrder'), href: route('work-orders-status.index', status) },
  ];

  const onPage = (e: DataTablePageEvent) => {
    const page = (e.page ?? 0) + 1;
    router.get(route('work-orders-status.index', status), { page, search: filters }, { preserveScroll: true });
  };
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="p-4">
        ...
        <DataTable
          ...
          filterDisplay="row"
          globalFilterFields={['order_id', 'customer_name', 'product']}
      ...
      >
        ...
        >
          ...
          <Column
            ...
            body={(rowData) => {
              // If rowData includes a related operator object, prefer its name
              console.log(rowData);
              if (rowData.operator?.name) {
                return rowData.operator.name;
              }

              // Fallback: match by operator_id from dropdown options
              const operator = operatorOptions.find(
                (opt: any) => opt.id === rowData.operator_id
              );
